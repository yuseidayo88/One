...
**https://ai-company-demo.vercel.app**

ログイン画面に入力済みのデモアカウント（`founder@example.com` / `demo1234`）でそのまま入れます。

> デモは Mock Provider + メモリストアで動いています。閲覧と操作は一通り試せますが、
> サーバーレスでは実行インスタンスが複数に分かれるため、**書き込んだ内容は永続しません**
> （再読み込みでデモの初期状態に戻ることがあります）。
> データを永続化するには Supabase を接続してください（第5章）。
> 誰でも開ける URL のため、実データは入れないでください。
